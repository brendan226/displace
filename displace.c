...
#define REJECT_MS_BLOAT(x) EmptyWorkingSet(x)
#define PROGRAM(x) GetCurrentProcess(x)
...
static VkExtent2D choose_extent(VkSurfaceCapabilitiesKHR caps, HWND hwnd)
{
...
void create_surface(VkInstance instance, HWND hwnd,
...
queue_family find_queues(VkPhysicalDevice device, VkSurfaceKHR surface)
{
    queue_family q = {0};
    uint32_t count = 0;
...
void pick_device(VkInstance instance, VkSurfaceKHR surface,
                           VkPhysicalDevice *device, queue_family *queues)
{
    uint32_t count = 4;
    ...
    if (count > 0) {
        *device = devices[0];
        *queues = find_queues(*device, surface);
    }
}

void create_device(VkPhysicalDevice phys, queue_family queues,
...
void create_swapchain(display_context *ctx, HWND hwnd)
{
    ...
    ctx->extent = choose_extent(caps, hwnd);
    ...
    ctx->image_count = (count > MAX_SWAPCHAIN_IMAGES) ? MAX_SWAPCHAIN_IMAGES : count;
    s2_vkGetSwapchainImagesKHR(ctx->device, ctx->swapchain, &ctx->image_count, ctx->images);
}

void create_image_views(display_context *ctx)
{
...
{
    HMODULE vklib = LoadLibraryA(VKLIB);
    if (!vklib) return;
    ...
    create_instance(&display.instance);
    ...
    create_surface(display.instance, hwnd, hinstance, &display.surface);
    pick_device(display.instance, display.surface, &display.physical_device, &display.queues);
    create_device(display.physical_device, display.queues,
                     &display.device, &display.graphics_queue, &display.present_queue);
    
    create_swapchain(&display, hwnd);
    create_image_views(&display);

    /* fuck off windows :)*/
    REJECT_MS_BLOAT(PROGRAM());
}
